...
class BiDirEncoder(nn.Module):
	...
	def forward(self, inputs, lengths):
		...
		input_dim = inputs.data.shape[2]

		input_sorted, sorted_lengths, original_indices = utils.sort_batch(inputs, lengths) #input_sorted: batch_size*num_words*input_dim
		...
		rnn_out, _ = self.rnn(iembeds, hidden) #rnn_out: num_words*batch_size*num_directions.hidden_dim
		...
		return rnn_out, None #rnn_out: batch_size*num_words*num_directions.hidden_dim, #lengths: <=batch_size
...
if __name__=='__main__':
	...
	rnn_out, lengths_new = net(inputs, lengths)
	print (lengths_new)
	print (rnn_out)
